...
JNIEXPORT int JNICALL Java_com_jmesh_controler_jni_CenterAirConditionerConnector_getAirConditionerStatus(
        JNIEnv *env,
    ...
    data_pack_getZhongYangKongTiao(&in, &output_stru);

    (*env)->SetByteArrayRegion(env, dataResult, 0, strlen(output_stru.frame), output_stru.frame);
    return strlen(output_stru.frame);
}

JNIEXPORT int JNICALL Java_com_jmesh_controler_jni_CenterAirConditionerConnector_resolveAirConditionerStatus(
        JNIEnv *env,
    ...
    int err_len = 0;
    OUTPUT_STRU output_stru = {data, frame, error, data_len, frame_len, err_len};
    int flag = data_result_getZhongYangKongTiao(&input_stru, &output_stru);
