...
JNIEXPORT int JNICALL Java_com_jmesh_controler_jni_MeterJniConnecter_transCmd(
    ...
    data_pack_cmd(&in, &output_stru);

    (*env)->SetByteArrayRegion(env, dataResult, 0, output_stru.frame_len, output_stru.frame);
...
JNIEXPORT int JNICALL Java_com_jmesh_controler_jni_MeterJniConnecter_dataResult(
    ...
    int err_len = 0;
    OUTPUT_STRU output_stru = {data, frame, error, data_len, frame_len, err_len};
    int flag = data_result_cmd(&input_stru, &output_stru);
