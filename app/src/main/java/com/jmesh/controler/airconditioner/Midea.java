 ...
 * 美的空调红外码
 */
public class Midea implements IAirConditioner {
    /*
    ...
    private static String getMiderC(String[] zl) {
        String models = Midea.getModel(zl);
        String temp = Midea.getTemp(zl);
    ...
    private static String getReverseC(String[] zl) {
        String codec = "";
        String former = Midea.getMiderC(zl);
    ...
    private static String getMiderB(String[] zl) {
        String speeds = Midea.getSpeed(zl);
        String bm = "F" + speeds;
    ...
    private static String getReverseB(String[] zl) {
        String codeb = "";
        String former = Midea.getMiderB(zl);
        int intformer = Integer.parseInt(Integer.valueOf(former, 16).toString());//16进制转换为10进制
        String intform = Integer.toBinaryString(intformer);//10进制转换为2进制
        //System.out.println("");
        //补齐8位后才计算补码
    ...
    public static String getMiderKthw(String zl) {
        ...
        String Bm = Midea.getMiderB(zllist);
        String Cm = Midea.getMiderC(zllist);
        String reverseB = Midea.getReverseB(zllist);
        String reverseC = Midea.getReverseC(zllist);
    ...
    public String getCode(AirConditionerData airConditionerData) {
        ...
        switch (airConditionerData.getMode()) {
            case AirConditionerData.modeHeat:
                stringBuilder.append("制热");
                break;
            case AirConditionerData.modeHumidification:
                stringBuilder.append("抽湿");
                break;
            case AirConditionerData.modeRefrigeration:
                stringBuilder.append("制冷");
                break;
            case AirConditionerData.modeSimpleBlow:
                stringBuilder.append("送风");
                break;
            case AirConditionerData.modeAuto:
                stringBuilder.append("自动");
                break;
        }
        stringBuilder.append(",");
        stringBuilder.append(Integer.toString(airConditionerData.getTemperture()));
        stringBuilder.append(",");
        switch (airConditionerData.getWindStrength()) {
            case AirConditionerData.windStrengthLowLevel:
                stringBuilder.append("一级");
                break;
            case AirConditionerData.windStrengthMidLevel:
                stringBuilder.append("三级");
                break;
            case AirConditionerData.windStrengthHighLevel:
                stringBuilder.append("五级");
                break;
            case AirConditionerData.windStrengthAuto:
                stringBuilder.append("自动");
                break;
        }
        return getMiderKthw(stringBuilder.toString());
    }
}
